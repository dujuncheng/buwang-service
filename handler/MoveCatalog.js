...
class MoveCatalog extends BaseClass {
  ...
  async run(ctx, next) {
    try {
      const paramOk = this.checkParams(['catalog_id', 'parent_id']);
      ...
      if (this.param.parent_id !== 0) {
        const parentArr = await this.CatalogModel.getArrByCatalogId(this.param.parent_id, this.uid);
      ...
      const updateRes = await this.CatalogModel.moveCatalog(this.param.catalog_id, this.uid, this.param.parent_id);
      if (updateRes) {
        ctx.body = {
          success: true,
          message: '移动成功',
